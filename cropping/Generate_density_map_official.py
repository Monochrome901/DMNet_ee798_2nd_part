...
def compute_distances(out_dist_path='distances_dict.pkl', raw_label_dir='.', n_neighbors=10,
...
if __name__ == "__main__":
    ...
    # Path to save the ground truth density maps
    ground_truth_dir = os.path.join(root_dir, mode, 'ground_truth')
    
    # Create the 'ground_truth' directory if it doesn't exist
    ...
    with open(str(mode) + ".txt", "w") as fileloader:
        # Prepared for the training algorithms that require a txt output file
        ...
        for img_path in tqdm(img_paths):
            ...
            ann_path = img_path.replace(img_affix, '.txt')
            ann_path = ann_path.replace("images", 'annotations')
            # Load image and ground truth points
            ...
            density_map = cv2.resize(density_map, (width//4, height//4))
            # Save density map in the 'ground_truth' directory
            den_name = os.path.join(ground_truth_dir, os.path.basename(img_path).replace("jpg", "npy"))
            if showden:
                plt.imshow(img)
